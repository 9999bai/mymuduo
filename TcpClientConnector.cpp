...
TcpClientConnector::TcpClientConnector(EventLoop* loop, const InetAddress& serverAddr)
                : AbstractConnector(mymuduo::CheckLoopNotNull(loop), serverAddr)
...
void TcpClientConnector::connect()
{
...
void TcpClientConnector::connecting(int sockfd)
{
    setState(kConnecting);
    channel_.reset(new Channel(loop_, sockfd));
    channel_->setWriteCallabck(std::bind(&TcpClientConnector::handleWrite, this));
    channel_->setErrorCallback(std::bind(&TcpClientConnector::handleError, this));
    channel_->enableWriting();
}

void TcpClientConnector::handleWrite()
{
    if(state_ == kConnecting)
    {
        ...
        if(err)
        {
            LOG_ERROR("TcpClientConnector::getSocketError errno=%d\n", err);
            retry(sockfd);
        }
        else
        {
            ...
            {
                sockets::close(sockfd);
                LOG_INFO("TcpClientConnector close...");
            }
        }
    }
}

void TcpClientConnector::handleError()
{
    LOG_ERROR("TcpClientConnector::handleError state=%d\n", (int)state_);
    if(state_ == kConnecting)
    {
        int sockfd = removeAndResetChannel();
        int err = sockets::getSocketError(sockfd);
        LOG_INFO("TcpClientConnector::handleError err=%d\n",err);
        retry(sockfd);
    }
}
